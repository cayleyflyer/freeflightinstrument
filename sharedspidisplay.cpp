...
void SharedSPIDisplay::drawWindIndicator() {
  int16_t x0, y0, x1, y1, x2, y2;
  x0 = 360 - POSITION_MARKER_SIZE;
  y0 = 190 + POSITION_MARKER_SIZE;
  x1 = 360;
  y1 = 190 - 2 * POSITION_MARKER_SIZE;
  x2 = 360 + POSITION_MARKER_SIZE;
  y2 = 190 + POSITION_MARKER_SIZE;
  disp.fillTriangle(x0, y0, x1, y1, x2, y2, BLACK);
};
...
void SharedSPIDisplay::drawDataWindow() {
  disp.fillRect(250, 0, 150, 240, WHITE);
  draw_line(250, 0, 250, 240, 4, BLACK);
  disp.setTextSize(1);
  disp.setTextColor(BLACK);
  disp.setCursor(256, 2);
  disp.write("Altitude (ft)");

  disp.setTextSize(1);
  disp.setTextColor(BLACK);
  disp.setCursor(256, 50);
  disp.write("Heading (deg)");

  disp.setTextSize(1);
  disp.setTextColor(BLACK);
  disp.setCursor(256, 100);
  disp.write("Speed kmh");
