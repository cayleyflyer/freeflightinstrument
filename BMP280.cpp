...
float BMP280::calculatedvsi(){
 float VerticalSpeed = VSIClass::NewVsi;
 return VerticalSpeed;
}

float BMP280::AltiMeters(float seaLevelhPa) {
  float altiM;
  float pressure = getPressure();  // in Si units for Pascal
  pressure /= 100;
  altiM = 44330 * (1.0 - pow(pressure / seaLevelhPa, 0.1903));
  return altiM;
}

float BMP280::AltiFeet(float seaLevelhPa) {
  float altiM;
  float altFeet;
  // float SmoothAlt;
  float pressure = getPressure();  // in Si units for Pascal
  ...
  altiM = 44330 * (1.0 - pow(pressure / seaLevelhPa, 0.1903));
  altFeet = altiM * 3.28084;
  // SmoothAlt = SmoothAlt - (0.025 * (SmoothAlt - altFeet));
  return altFeet;
}
